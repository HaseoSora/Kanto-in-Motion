...
-- Cooperative staged-battle overlay compatibility: preserve external scenes, modernize HUD/menus.
--
...
return function(mod)
  ...
  -- into the same ownership split external staged battle uses without being hard-coded by
  ...
  local optionSchema = {
    ...
    { key = "battleUiWip", label = "MODERN BATTLE UI", type = "toggle", default = true,
      description = "Enable Modern UI battle integration. KIM HD and cooperative external/voxel battles use Modern UI only for the lower command/move/message surface; source HP/status and EXP furniture remain untouched.", },
    { key = "battleUiScope", label = "BATTLE UI SCOPE", type = "choice",
      description = "Controls ordinary non-Kanto 2D battle presentation. KIM HD and cooperative external/voxel battles always use the hybrid lower-panel Modern UI while preserving the source HP/status/EXP HUD. ITEMS + POKEMON otherwise keeps FIGHT/moves native; FULL enables the complete experimental 2D overlay.",
      choices = { { "ITEMS + POKEMON", "items_party" }, { "FULL", "full" } },
      default = "items_party" },
    { key = "battle3dBypass", label = "LEAVE 3D BATTLES ALONE", type = "toggle", default = false,
      description = "ON makes Modern UI back out of detected external staged battle/3D/voxel battles completely. OFF replaces only their lower command/move/message surface while leaving the arena, camera, Pokemon, attack animations, HP/status HUD and EXP presentation untouched.", },
  ...
  -- Title CONTINUE and external staged battle precache/cache-loading screens are
  ...
  -- settings screens from the centralized MOD MENU. Calling the source row's
  -- activate closure preserves that mod context instead of asking KIM's
  -- ui.push helper to resolve another mod's screen.
  ...
  -- Battle Art 1.10+ / 1.11+ owns categorized option pages. Build its
  -- ordinary OptionsMenu model without pushing it, then reuse the exact
  -- source-owned group descriptors from KIM's centralized MOD MENU.
    ...
    local function addRow(row)
      ...
      if id:find("^BATTLE_ART_VOXEL_FORK:group:")
        ...
        items[#items + 1] = {
          ...
          keepOpen = true,
          onSelect = function() pcall(sourceRow.activate, game) end,
        }
      end
    end
    for _, row in ipairs(rows) do addRow(row) end
    -- KIM's OPTIONS wrapper intentionally hides rows represented by MOD MENU,
    -- but records the original source descriptors first. If the just-built
    -- menu is already filtered, recover Battle Art's group callbacks from
    -- that capture instead of falling back to a dead synthetic screen id.
    if #items == 0 then
      for _, row in pairs(runtime.capturedCentralOptionRows or {}) do addRow(row) end
    end
    ...
    for _, screen in ipairs((battleArtCategories and #battleArtCategories > 0)
      ...
      items[#items + 1] = {
        ...
        onSelect = function()
          ...
          local opened = false
          -- Source-owned settings use the source mod's OPTIONS callback so its
          -- own UI context and dynamic visibility remain intact.
  ...
  local OPTION_CATEGORY_ORDER = {
    { id = "kanto", label = "KANTO IN MOTION",
      description = "HD animated Pokemon, HD battle backgrounds, title presentation, and bundled UI control." },
  ...
  -- the external scene owner's persistent voxel utilities intentionally expose ordinary
  ...
  -- importing external staged battle internals or creating a hard dependency.
  ...
  function battleRuntime.nativeSceneRequested(state)
    -- Presentation selection is automatic. KIM HD publish their own
    -- Battle Lite ownership flag, while staged external/voxel battles are
  ...
  -- Gen1Recomp's Pokédex does not push a separate Dex screen when A is
  -- pressed on a known species. It pushes a normal Menu whose *instance*
  -- draw function is replaced by PokedexMenu's tiny legacy side-cursor
  -- renderer (DATA / CRY / AREA / [PRNT] / QUIT).  The generic custom-draw
  -- safety check therefore treated that perfectly ordinary submenu as an
  -- unknown UI owner and restored the entire classic Pokédex underneath it.
  --
  -- Recognize that audited side menu by its stable structure rather than its
  -- English labels so localization mods remain compatible. Modern UI can then
  -- keep the rich Pokédex page visible and draw the side choices as a normal
  -- modern modal card.
  runtime.isPokedexSideMenu = function(state)
    if type(state) ~= "table" then return false end
    if state._gen1ModernPokedexSideMenu == true then return true end
    if not inherits(classOf(state), menuClass) then return false end

    local items = state.items
    if type(items) ~= "table" or (#items ~= 4 and #items ~= 5) then
      return false
    end

    -- PokedexMenu.chooseEntry fixes this shell to column 14 / width 6 and
    -- replaces Menu:draw on the instance after setting th = rows*2+2.
    if tonumber(state.tx) ~= 14 or tonumber(state.tw) ~= 6
        or tonumber(state.th) ~= (#items * 2 + 2) then
      return false
    end
    if type(rawget(state, "draw")) ~= "function" then return false end

    -- DATA, AREA, QUIT (and Yellow's PRNT) close normally; CRY is the one
    -- keep-open action. That pattern is stable even when every label changes.
    if type(items[1]) ~= "table" or type(items[1].onSelect) ~= "function"
        or type(items[2]) ~= "table" or items[2].keepOpen ~= true
        or type(items[2].onSelect) ~= "function"
        or type(items[3]) ~= "table" or type(items[3].onSelect) ~= "function"
        or type(items[#items]) ~= "table"
        or type(items[#items].onSelect) ~= "function" then
      return false
    end

    -- The parent Pokédex marks its selected row hollow while this side menu
    -- is open. When available, use that as an additional proof so no unrelated
    -- six-column Menu can be mistaken for the Dex submenu.
    local game = state.game
    local stack = game and game.stack
    if stack and type(stack.states) == "table" then
      local states = stack.states
      for i = #states - 1, 1, -1 do
        local parent = states[i]
        if type(parent) == "table"
            and parent.screenId == "PokedexMenu"
            and parent.hollowIndex ~= nil then
          state._gen1ModernPokedexSideMenu = true
          return true
        end
      end
    end

    -- Older stack implementations do not expose .states publicly. The exact
    -- shell + action pattern above is still specific enough to the stock Dex.
    state._gen1ModernPokedexSideMenu = true
    return true
  end
  ...
  runtime.battleUiScope = function()
    ...
    -- Kanto in Motion's KIM-style 2D battle system (or its master OFF
  ...
  runtime.battlePresenterActive = function(game, state)
    ...
    -- surfaces while Kanto in Motion keeps the native/external staged battle HP HUD.
    ...
    -- Kanto in Motion Battle Lite is a deliberate hybrid: KIM/KIM-style
    ...
    if native3d then
      -- Legacy external/voxel publishers that predate the open registry keep
  ...
  runtime.rowsFor = function(game, state, kind)
    ...
    if kind == "menu" and runtime.isPokedexSideMenu
        and runtime.isPokedexSideMenu(state) then
      title = Strings("POKéDEX")
  ...
  runtime.drawTrainerCard = function(game, state, viewport, theme)
    ...
    love.graphics.rectangle("fill", portraitX, portraitY, portraitSize, portraitSize,
      theme.radii.sm)
    -- Keep the native Trainer Card player portrait. KIM customizes only the
    -- earned badge artwork in this scan-clean build.
    ...
    -- Earned badges use KIM's animated badge art; unearned slots fall back
    -- to the game's native Trainer Card face/badge sheet.
  ...
  function battleRuntime.presentationMode(state, model)
    ...
    -- overlays can share external staged battle geometry. That compatibility table does
    ...
    -- A staged external/voxel battle keeps its complete scene and source
    -- HP/status HUD. Modern UI owns only the lower command/move/message strip,
    -- matching KIM HD instead of entering the old scene-HUD status-card path.
  ...
  function battleRuntime.lowerPanelTheme(theme, state)
    ...
    if not kimBattle and not cooperativeLower and mod._gen1ModernCompatibility
      ...
      native3dFallback = mod._gen1ModernCompatibility:isNative3dBattle(
        game, sourceState or state) == true
    end
    -- KIM HD, external staged battle, and cooperative LOWER battle integrations share
    -- Kanto in Motion's BATTLE TEXT SIZE unless the source opts out.
    if not (kimBattle or native3dFallback or cooperativeLower) then return theme end
  ...
  -- Keep PP readable on Typed Move Colors tiles. Desktop already uses
  -- this contrast-aware rule; mobile had fallen back to textMuted, which
  -- becomes pale pink/gray on bright move tiles and is difficult to read.
  function battleRuntime.movePpColor(typedStyle, selected, theme)
    ...
    if type(bg) == "table" then
      local r, g, b = tonumber(bg[1]) or 0, tonumber(bg[2]) or 0,
        tonumber(bg[3]) or 0
  ...
  runtime.drawBattleKimMoves = function(game, state, layoutTheme, textTheme,
    ...
    for index = 1, 4 do
      ...
      love.graphics.setFont(captionFont)
      setColor(battleRuntime.movePpColor(
        typedStyle, index == selected, layoutTheme))
  ...
  runtime.drawBattle2dMoves = function(game, state, theme, x, y, w,
    ...
    for index = 1, slotCount do
      ...
      love.graphics.setFont(captionFont)
      setColor(battleRuntime.movePpColor(
        typedStyle, index == selected, theme))
      drawText(entryPP, cellX + cellW - spacing.sm - ppW,
        cellY + (rowH - textHeight(captionFont)) / 2)
      local effect = state.phase == "moveSelect" and entry
        and battleRuntime.typedMoveEffect(game, state, entryDefinition) or nil
  ...
  runtime.drawBattle2dHud = function(game, state, viewport, theme, model, options)
    ...
    local kimLowerPanel = (kimLowerState
      ...
      or (game and game._kantoInMotionFullscreenBattle == true)
    -- the external scene owner's hybrid lower strip uses the same fixed KIM HD geometry:
    ...
    -- Gen 1 HUD restyled like external staged battle. Modern UI owns commands/messages,
    ...
    if isTop and fullscreenLowerPanel then
      ...
      -- portrait top/bottom theme bands meet the HD image exactly instead of
      ...
      -- HD battle backgrounds are 1920x950. When Kanto in Motion lifts that
      ...
      -- clear colour. The edge comes from main.lua's live HD transform, so the
      -- band automatically adapts to 16:9, 4:3 and ultrawide windows.
      local krsFooterTop = (kimFullscreen and not kim2DBattle) and (
        tonumber(source and source._kantoInMotionHdFooterTop)
        or tonumber(native and native._kantoInMotionHdFooterTop)
        or tonumber(state and state._kantoInMotionHdFooterTop)) or nil
      ...
      if krsFooterActive then
        ...
        if touchBattleControlsVisible(game) and orientation == "landscape" then
          -- Keep the established HD footer *height* exactly as before, but
      ...
      local sharedFullscreenPanel = mobileRect == nil
        and (options.sharedFullscreenPanel == true or kimFullscreen)
      if sharedFullscreenPanel then
        -- User mock reference: KIM HD uses the same intentionally oversized lower UI
        ...
        -- panel edge-to-edge and all the way to the physical bottom. The HD
      ...
      if type(mobileRect) == "table" then
        -- Mobile portrait/landscape has an authored native command/dialog
        -- rectangle published by main.lua. Keep that exact footprint at 100%
        -- so Modern UI stays in the same accepted location for both normal
        -- KIM battles and external staged battle 3D-BTL. The large-font fitting added for
        -- desktop must never grow this mobile rectangle upward.
        --
        -- BATTLE UI SIZE may still compact the panel, but—as in the previously
        -- confirmed mobile layout—it remains bottom-anchored and is never
        -- allowed to become taller than the native mobile rectangle.
        ...
        -- Desktop/non-authored layouts can grow upward when a large pixel font
        -- needs more room. This preserves the v36 font-fitting repair without
        -- moving the established mobile battle dialog.
        local panelBottom = panelY + panelH
        local minPanelH = battleRuntime.lowerPanelMinimumHeight(
          game, theme, textTheme, isMove)
  ...
  runtime.drawBattleHud = function(game, state, viewport, theme, model, mode)
    ...
    if mode == "lower" then
      -- external/voxel hybrid: use the exact KIM HD lower strip while the
  ...
  runtime.drawBattle = function(game, state, viewport, theme)
    ...
    -- the full framed composition, while staged external/voxel battles use
  ...
  function battleRuntime.scrubNativeUi(game, ctx, layers, renderer)
    ...
    -- classic canvas with HD arena + Pokemon + native attack animations and omitted
  ...
  function battleRuntime.ensureDecoratedState(game, decorated)
    ...
    if childOnly then
      ...
      -- the modern panel, and make the state transparent so the external scene owner's live
  ...
  -- source keeps status/HP/EXP/world/battlers/effects exactly as external staged battle
  -- does. A source may narrow/expand that set with suppressSurfaces.
  -- Legacy external staged battle detection remains as a compatibility fallback.
