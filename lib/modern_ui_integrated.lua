...
return function(mod)
  ...
  local optionSchema = {
    ...
    { key = "battleUiWip", label = "MODERN BATTLE UI", type = "toggle", default = true,
      description = "Enable Modern UI battle integration. BATTLE UI SCOPE chooses whether only in-battle Items/Pokemon or the full battle overlay is modernized.", },
    { key = "battleUiScope", label = "BATTLE UI SCOPE", type = "choice",
      description = "ITEMS + POKEMON keeps FIGHT/move selection native for mods such as Typed Move Colors, while modernizing supported non-FIGHT battle child flows including Bag, Party, level-up stats, caught Dex, nickname, evolution, and move-learn. FULL enables the complete experimental battle overlay.",
    ...
    { key = "battle3dBypass", label = "LEAVE 3D BATTLES ALONE", type = "toggle", default = false,
      description = "ON makes Modern UI back out of detected 3D/voxel battles completely. OFF allows Modern UI to participate according to BATTLE UI SCOPE while leaving the 3D arena, camera, Pokemon placement, and attack animations untouched.", },
  ...
  runtime.battleUiScope = function()
    local value = runtime.option("battleUiScope", "items_party")
    return value == "full" and "full" or "items_party"
  end
  ...
  runtime.battlePresenterActive = function(game, state)
    ...
    if native3d then
      -- Battle Art/voxel battles keep complete ownership of the 3D scene,
      -- Pokemon cards, camera, and attack animation layer.  When the safety
      -- bypass is disabled, Modern UI owns only the HUD/menu/text overlay.
      -- Do not require the source BattleState to advertise the ordinary 2D
      -- WIDE layout: staged voxel battles intentionally retain the classic
      -- source state while presenting a widescreen scene of their own.
      return runtime.option("battle3dBypass", false) ~= true
    end

    -- Ordinary 2D battles retain the existing safety rule: the experimental
    -- presenter only owns an explicitly identified WIDE source.
  ...
  function battleRuntime.presentationMode(state, model)
    local game = runtime.ownerGame(state, currentGame)
    -- A staged voxel/3D battle needs only the Modern UI overlay.  Returning
    -- HUD mode preserves Battle Art's complete scene while replacing its
    -- native status/menu/text surfaces with the themed Modern UI versions.
  ...
  -- Battle Art exposes a presentation-suppression hook. In ITEMS + POKEMON
  -- scope claim only its command/text panels while an in-battle Bag/Party
  -- child is open. The status HUD stays visible, and when the child closes we
  -- claim nothing, so Typed Move Colors and the native FIGHT/move renderer are
  ...
  mod.hooks:wrap("battle.presentation.suppress_native.v1",
    function(next, request)
      ...
      if runtime.battleUiScope() ~= "items_party"
