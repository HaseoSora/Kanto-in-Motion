...
return function(mod, isBattleShiny, directStageGeometry, directSideMetrics)
  ...
  -- External staged-scene compatibility. The scene owner controls the window-resolution
  -- arena, while the shiny cue remains an independent KIM overlay. Its live
  ...
  function M:drawExternalStage(battle, game)
  ...
  -- Android/iOS external-stage overlay. The desktop path uses push/pop to
  ...
  function M:drawExternalStageMobile(battle, game)
    if type(battle) ~= "table" then return false end
    local shot = battle.dramaticShapeShot
    if type(shot) ~= "table" or not shot.player or not shot.enemy
        or not tonumber(shot.scale) then return false end
