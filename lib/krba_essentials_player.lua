...
return function(mod, DATA)
  ...
  function BattleState:drawPicsLayer(slide,sx,sy,onlySide,skipMenuClip)
    ...
    if not onlySide then
      ...
      if not (baStage and battleArtPlaneApi) then sess:drawBackground() end
  ...
  function BattleState:drawAnimLayer(colorized)
    ...
    -- With Battle Art API v2 the full-stage compositor owns FG at native
    -- scene resolution. Older Battle Art builds keep the previous logical
    -- screen-space fallback rather than losing the plane entirely.
    if not battleArtPlaneApi then sess:drawForeground() end
    return result[2]
  end
