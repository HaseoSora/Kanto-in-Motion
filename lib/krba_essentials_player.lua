...
return function(mod, DATA)
  ...
  local function newSession(owner, anim, moveId, attackerIsPlayer, enemyFallbackMode, opponentVariant)
    local s = {
      ...
      wideAnchorMode=classifyEnemyFallback(anim),
    ...
    -- KIM Wide/Gen6 uses battler positions that no longer match Essentials'
    -- canonical 512x307 stage. Previously only EMBER was retargeted to KIM's
    -- real battler anchors, so target-local moves such as SLASH/GUST and impact
    -- moves such as TACKLE still landed at the old canonical location.
    --
    -- Full-field animations keep their authored stage mapping. Every other
    -- effect uses the animation's source geometry classification:
    --   target  -> preserve its local offsets around the real target;
    --   user    -> preserve its local offsets around the real user;
    --   reflect -> travelling/mixed sequence; remap USER->TARGET continuously.
    ...
    function s:drawCellWide(c, kimAnchors)
      ...
      local g=love.graphics
      local oldMode,oldAlpha=g.getBlendMode()
      local kimAnchored = kimAnchors and not self.wideStageCelScale
      local celBlend=blendMode(c[7])
      ...
      local cx,cy=(c[1] or 0),(c[2] or 0)
      if kimAnchored then
        local userSide = self.attackerIsPlayer and "player" or "enemy"
        ...
        -- KIM draws the battlers with Essentials' -1/-2 role-cell transforms
        -- (hit shake, lunge, etc.).  Aim the particles at those same live
        -- transformed centers rather than at the untransformed base position.
        local function transformedAnchor(anchor, side)
          local out = { x=anchor.x or 0, y=anchor.y or 0 }
          if type(self.battlerTransformWide)=="function" then
            local t=self:battlerTransformWide(side)
            if type(t)=="table" then
              out.x=out.x+(tonumber(t.dx) or 0)
              out.y=out.y+(tonumber(t.dy) or 0)
            end
          end
          return out
        end
  ...
  local function kantoReworkEnabled(animPlayer)
    ...
    if mod and mod.options and type(mod.options.get) == "function" then
      ...
      if okSystem and systemEnabled == false then
        ...
        -- while its real 3D-BTL stage is active.
        if not allowed then
