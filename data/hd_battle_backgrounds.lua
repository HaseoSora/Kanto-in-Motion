-- Kanto in Motion HD battle-background router.
...
-- Coordinates are LOCAL TO THE BACKGROUND image (the HD battle presenter
...
-- HD family instead of falling back to the old generated colour
...
local function periodValue(value)
  ...
  if key:find("SUNSET",1,true) or key:find("DUSK",1,true)
      or key=="EVE" or key:find("EVENING",1,true)
      or key:find("GOLDEN",1,true) then return "sunset" end
  if key=="NITE" or key:find("NIGHT",1,true) or key=="DARK" or key=="NITE_F" then return "night" end
  return "day"
end

function M.period(game)
  -- Graphics owns the live game clock. Read it directly so a battle entered
